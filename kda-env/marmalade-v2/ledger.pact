(module ledger GOVERNANCE
  ...
  (defun enforce-transfer-policy
    ...
    (let ((token (get-token-info id)))
      (policy-manager.enforce-transfer token sender (account-guard id sender) receiver amount))
  )
  ...
  (defun burn:bool
    ...
    (with-capability (LEDGER)
      (with-capability (BURN id account amount)
        (let ((token (get-token-info id)))
          (policy-manager.enforce-burn token account amount))
        (let
          (
  ...
  (defun offer:bool
    ...
    (let ((token (get-token-info id)))
      (policy-manager.enforce-offer token seller amount (pact-id)))
    (let
      (
  ...
  (defun withdraw:bool
    ...
    (require-capability (SALE_PRIVATE (pact-id)))
    (let ((token (get-token-info id)))
      (policy-manager.enforce-withdraw token seller amount (pact-id)))
    (let
      (
  ...
  (defun buy:bool
    ...
    (require-capability (SALE_PRIVATE (pact-id)))
    (let ((token (get-token-info id)))
      (policy-manager.enforce-buy token seller buyer buyer-guard amount sale-id))
    (let
      (
        (sender (debit id (sale-account) amount))
